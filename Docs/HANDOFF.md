...
| A5 | `no_std` 口径（D1）／trait 动态性（D2）／算法枚举扩展方式（D3）／后端选择机制（D4） | 见 [`DESIGN.md`](DESIGN.md) §2 | ✅ **已决（2026-09-16）**：D1=(a) 合同层 `no_std`+`alloc` 后端不限；D2=(d) 单层对象安全合同（工厂化构造）；D3=(a) 封闭 `#[non_exhaustive]` 枚举；D4=(e) 纯构造注入（合同零选择机制）；D5=(d) 统一分类枚举＋源链；D6=(b) in-tree stub；D7=(d) 单清单内容寻址向量架构 |
| A6 | "做到 OpenSSL 的水平"的目标声明与生态位 | 用户 2026-09-16 提出：功能覆盖度对标＋有现成纯 Rust 实现则引用 | ✅ **已决（2026-09-16 全流程定案）**：生态位＝方案 A（集成供给层）＋双锚点（获客=验证级稀缺件，留存=统一合同）＋"OpenSSL 对标面＋现代生态超集"＋取代 aws-lc-rs（FIPS 段永不碰）＋对外叙事收敛；内容清单经加法轮/补漏轮/复核轮/减法轮定稿于 [`CONTENT.md`](CONTENT.md)（15 crate、自研 ~5k 行）；目标声明见 [`GOALS.md`](GOALS.md) |
