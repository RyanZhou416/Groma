...
1. **不重写已有纯 Rust 算法**（RustCrypto／graviola／libcrux 已有 200+ crate）。
2. **不移植其他语言的参考实现源码**；一律**照规范＋测试向量**实现。
3. **不自己实现椭圆曲线／RSA 大数／哈希／随机数**，用经过审查的 crate 并记录其审查边界。
4. **每个不可信解析入口先限长再解析**；失败关闭；错误可诊断；**不记录密钥材料或秘密**。
5. **不得未经独立审计就宣称**"生产安全""已审计""可替代 OpenSSL"；形式化验证只按其真实覆盖表述。
6. **FIPS 140-3 是显式非目标**，不得暗示已合规。
7. 密码学改动**必须有公开向量 ＋ 至少一个负向用例**；新增解析入口必须带 fuzz target。截图／手工登录／单浏览器演示**不是安全门**。
8. 交付路径**不得引入 C/C++ 构建要求**。
9. 范围变更**必须先改 `SCOPE.md`**，不能靠"顺手加个算法"扩范围。
...
| A5 | `no_std` 口径（D1）／trait 动态性（D2）／算法枚举扩展方式（D3）／后端选择机制（D4） | 见 [`DESIGN.md`](DESIGN.md) §2 | P1 必须定，晚了改不动；**D1 已决（2026-09-16）：方案 (a) 合同层 `no_std`+`alloc`、后端不限**，D2–D4 待定 |
| A6 | "做到 OpenSSL 的水平"的目标声明与生态位 | 用户 2026-09-16 提出：功能覆盖度对标＋有现成纯 Rust 实现则引用 | 🔶 部分已决：生态位＝方案 A（集成供给层），见 [`POSITIONING.md`](POSITIONING.md) §2；目标声明（[`GOALS.md`](GOALS.md)）与阶段顺序、并入 SCOPE 时机待全貌后定 |
