...
| 2 不移植源码、照规范＋向量实现 | **修改（降级）** | "照规范＋公开向量"是行业共识（Go ML-KEM 实践）；"绝不移植"与行业主流相悖（BoringSSL 是 fork、ring 继承血统）；且 #2 反成陷阱（KyberSlash 就是官方参考实现中招——照规范+功能向量不测时序） | 改为："照规范实现＋外部公开向量＋负向用例"为硬性；"不移植"从禁令降为默认偏好（确需移植时逐文件许可审查＋SPDX 留痕），与 SCOPE 原文对齐 |
...
- [ ] #2 降级："禁止移植"→"照规范＋外部向量为硬性，移植为需审查的例外"
