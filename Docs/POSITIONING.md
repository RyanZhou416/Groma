...
>
> **获客锚——无 C 构建的密码学稀缺件**：当开发者需要 PKCS#12 创建、CMS 验签、OCSP 检查、TPM 验证、X.509 路径构建的纯 Rust 实现，需要无 C 构建的 PQC（ML-KEM/ML-DSA），或需要多协议栈共用的可替换后端时，Groma 是唯一去处。
>
...
- [ ] 开发内容设定：**待办**——将依据本生态位（双锚点）另行设定（用户指示"根据生态位设定开发内容"）
