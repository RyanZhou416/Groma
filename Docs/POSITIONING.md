...
>
> **Groma 不是"现有库的组合"**——那是 RustCrypto facade crate `crypto` 已证明失败的路径；组合不产生使用理由，稀缺件与合同才产生。

否定式定位同样重要——**Groma 不是**：
又一个算法实现集合（RustCrypto 已垄断基元层）／ 现有库的打包门面 ／ TLS 协议栈 ／ FIPS 140-3 模块 ／ HSM ／ aws-lc-rs 克隆（那意味着带 C）／ orion 式"自家实现全家桶"（我们引用生态，不自研算法）。
...
### 1.1 开发者视角的七个时刻（获客/留存钩子，2026-09-16 用户驱动分析）

"组合现有库"不产生使用理由——RustCrypto 自家 facade crate `crypto` 三年无发布即证明。Groma 的存在理由按"**开发者何时会想到它**"重述：

| # | 时刻（开发者的处境） | 今天他怎么办 | Groma 给什么 | 钩子类型 | 强度 |
|---|---|---|---|---|---|
| 1 | 必须换后端（FIPS 政策／无 C 政策／嵌入式目标） | 重写所有调用点 | 配置级切换＋能力查询 | 留存 | 强（新项目不会为"将来可能换"而选） |
| 2 | **需要的能力纯 Rust 不存在或被 C 锁死**（PKCS#12 创建／CMS 验签／OCSP 检查／TPM 验证／路径构建） | 没有选择：上 C 或没有 | 唯一供给 | **获客** | **最强（被缺件逼进来）** |
| 3 | 要 PQC 但不想碰 C（ML-KEM/ML-DSA 且无 aws-lc-rs） | DIY：ml-kem＋libcrux＋rustls 接缝自己焊 | 开箱即用 | **获客** | 强（窗口期） |
| 4 | 多协议栈共用一个后端（rustls＋quinn＋boringtun 的 ring/aws-lc-rs 冲突） | 版本冲突地狱 | 一个后端喂多个栈 | **获客** | 中强 |
| 5 | 要统一审计／合规证据 | 自己拼 15 个 crate 的证据 | 一份 dossier | 获客 | 中（是承诺非功能，不作主钩子） |
| 6 | 版本世代地震（digest 0.10→0.11 一次性打断全图） | 自己跟着修 | 合同吸收世代更替 | 留存 | 强 |
| 7 | 嵌入式与服务器同一套代码 | 两套实现 | 合同层 no_std＋alloc（D1 的意义） | 留存 | 中 |

**结论（双锚点）**：
- **获客锚 = 无 C 构建的密码学稀缺件（时刻 2/3/4）；留存锚 = 统一合同（时刻 1/5/6/7）**。两条腿缺一不可，对外讲述时 2/3/4 在前。
- 战略推论：**第一件稀缺件落地 = Groma 第一次真正获客**；纯"合同＋两后端"发布对外无新价值。补缺不是陪衬，是获客引擎；合同是留存引擎。
- 反向验证：只装 sha2/aes-gcm 且永不换后端的开发者永远直接引用，Groma 对他无意义——他不是目标用户，无需讨好。
...
> **2026-09-16 已决**：方案 A 获选，并按 §1.1 调整为**双锚点表述**（获客=稀缺件，留存=合同）；具体开发内容将依据生态位另行设定。
...
## 5.1 竞争策略：取代 aws-lc-rs（2026-09-16 用户拍板）

**取代对象不是 rustls，是 rustls 底下的 aws-lc-rs**——rustls 是分销渠道（我们做它的 provider），aws-lc-rs 才是要替代的 C 层。同一 Groma 后端值可同时喂 rustls/quinn/russh/boringtun，一次替换四个 aws-lc-rs/ring 依赖。

**错位竞争表**：aws-lc-rs 的护城河＝FIPS 证书＋rustls 默认位＋性能＋成熟度（吃"合规与惯性"）；Groma 的护城河＝零 C＋no_std＋编排稀缺件＋开放合同＋覆盖广度（吃"自由与能力"）。**FIPS 段永远不碰（R6，用户定）；其余市场段错位竞争**；唯一正面交火点是"无 C＋PQC"——那里 Groma 有结构性优势（aws-lc-rs 的 PQC 必须带 C）。

**取代路径三步**：① 先吃边缘（无 C 诉求买家 ①③④⑤，编排件获客）→ ② PQC 无 C 组合成熟后切交叉市场（rustls 的 PQC 用户想脱 C 时只有 Groma 接得住）→ ③ FIPS 段永不进入。

**对外叙事收敛（用户拍板）**：对外只讲三样——获客件（验证级编排稀缺件）＋统一合同＋无 C 构建；合同覆盖件（secp256k1/BLS/HPKE/FROST/AES-KW 适配）不作为卖点（开发者直接引用上游同样成立），官方适配器标"低优先级/社区可做"，官方精力集中在获客件与 rustls 适配器。
...
- [x] 生态位表述：**已决（2026-09-16）＝ 双锚点**（获客锚=无 C 构建的密码学稀缺件，留存锚=统一合同），见 §0 与 §1.1；"组合现有库"被否定为卖点
- [x] 阶段顺序微调：**暂缓**——D1–D7 已全部拍板；顺序等全貌整体讨论（用户指示"最后定"）
- [x] 开发内容设定：**已完成**——见 [`CONTENT.md`](CONTENT.md)（22 crate、26 项内容、加法轮、补漏轮）
- [x] 竞争策略：**已决**——取代 aws-lc-rs（非 rustls）；FIPS 段永远不碰；对外叙事收敛（§5.1）
...
| 2026-09-16 | 用户驱动分析"开发者何时会想到 Groma"（七个时刻、获客/留存钩子）入档 §1.1；生态位表述据此调整为双锚点（获客=稀缺件，留存=合同），"组合现有库"明确为非卖点；开发内容将依据生态位另行设定。 |
| 2026-09-16 | 新增 §5.1 竞争策略（用户拍板）：取代对象=aws-lc-rs 而非 rustls（rustls 为分销渠道）；FIPS 段永远不碰；错位竞争＋取代路径三步；对外叙事收敛（获客件＋合同＋无 C 为主体，合同覆盖件降级为"低优先级/社区可做"）。 |
